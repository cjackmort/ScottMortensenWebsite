...
/* ---------- VELOCITY-DRIVEN MARQUEE (endlesstools.io) ---------- */
const marqueeTrack = document.querySelector('.marquee-track');
const marqueeWrap  = document.querySelector('.marquee-wrap');
let scrollPrev = window.scrollY, scrollVel = 0, mSpeed = 30;
window.addEventListener('scroll', () => { scrollVel = window.scrollY - scrollPrev; scrollPrev = window.scrollY; }, { passive: true });
(function tickMarquee() {
  mSpeed = lerp(mSpeed, clamp(30 - Math.abs(scrollVel) * 2.2, 5, 30), 0.1);
  if (marqueeTrack) marqueeTrack.style.animationDuration = mSpeed.toFixed(2) + 's';
  // Skew the container (not the track, which has the CSS animation transform)
  if (marqueeWrap) marqueeWrap.style.transform = `skewX(${clamp(-scrollVel * 0.35, -10, 10)}deg)`;
